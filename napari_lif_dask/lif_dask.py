    ...
    if number_of_channels > 1:
        ...
        for cindex , channel_name in enumerate(name_of_channels):
            if number_of_time_points > 1:
                ...
                if nz > 1:
                    arr = da.stack(
                        [
                            aics_img.get_image_dask_data('ZYX', S=0, C=cindex, T=0)
                        ]
                    )
                else:
                    arr = da.stack(
                        [
                            aics_img.get_image_dask_data('YX', S=0, C=cindex, T=0, Z=0)
                        ]
                    )
        ...
        if number_of_time_points > 1:
            ...
            if nz > 1:
                arr = da.stack(
                    [
                        aics_img.get_image_dask_data('ZYX', S=0, C=0, T=0)
                    ]
                )
            else:
                arr = da.stack(
                    [
                        aics_img.get_image_dask_data('YX', S=0, C=0, T=0, Z=0)
                    ]
                )
